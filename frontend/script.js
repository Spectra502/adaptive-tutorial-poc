document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Appends a media (GIF) element to the chat by searching for a specific tag.
     * @param {string} mediaTag - The specific tag (e.g., "Ampel_Rot").
     */
    function addMedia(mediaTag) {
        let mediaPath = null;

        // Search through the entire handbook object
        for (const chapterKey in handbookData) {
            const chapter = handbookData[chapterKey];
            if (chapter.media && Array.isArray(chapter.media)) {
                // Find the media item with the matching tag
                const foundMedia = chapter.media.find(m => m.tag === mediaTag);
                if (foundMedia) {
                    mediaPath = foundMedia.path;
        ...
        if (mediaPath) {
            ...
            console.warn(`No media found for tag: ${mediaTag}`);
        }
    }
    ...
    chatForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const mediaTagRegex = /\[SHOW_MEDIA: (.*?)\]/;
            const match = aiResponseText.match(mediaTagRegex);
            ...
            if (match) {
                ...
                // 1. Get the specific media tag (e.g., "Ampel_Rot")
                const mediaTag = match[1];
                console.log("Media Tag:", mediaTag);

                // 2. Remove the tag from the text
                aiResponseText = aiResponseText.replace(mediaTagRegex, '').trim();
                console.log("Cleaned message text:", aiResponseText);
                
                // 3. Add the clean text message
                addMessage(aiResponseText, 'assistant');
                // 4. Add the media
                addMedia(mediaTag); // <--- Wir übergeben jetzt das spezifische Tag
